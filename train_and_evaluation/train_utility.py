...
def retrieve_indices(dataset='3dfront', **data_size):
    if dataset == 'museums':
        if os.path.exists('indices/indices_museum_dataset.pkl'):
            indices_pickle = open('indices/indices_museum_dataset.pkl', "rb")
            ...
            with open('indices/indices_museum_dataset.pkl', 'wb') as f:
                pickle.dump(indices_pickle, f)
        return train_indices, val_indices, test_indices
    elif dataset == '3dfront':
        if os.path.exists('indices/indices_3dfront_dataset.pkl'):
            indices_pickle = open('indices/indices_3dfront_dataset.pkl', "rb")
            indices_pickle = pickle.load(indices_pickle)
            train_indices = indices_pickle["train"]
            val_indices = indices_pickle["val"]
            test_indices = indices_pickle["test"]
...
def get_entire_data(dataset='3dfront'):
    if dataset == 'museums':
...
def get_margin_tensor(indexes, relevance_info, margins, status, thresholds=None):
    if thresholds:
        if status == 0:
            t = thresholds[0]
